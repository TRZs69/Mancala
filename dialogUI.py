...
class Ui_Dialog(object):
    def setupUi(self, Dialog):
        ...
        plate = QtGui.QPixmap("wadah.png").scaled(
            self.label.width(), self.label.height()
        )
        self.label.setPixmap(plate)

        self.winningLabel.hide()
        self.winningLabel.setStyleSheet(
            "font: bold; font-size:48px; color: rgb(255, 0, 0)"
        )
        self.numLabels = []
        ...
        for j in range(14):
            ...
            for i in range(4):
                ...
                if i == 0:
                    label2.setPixmap(
                        QtGui.QPixmap("gem.png").scaled(label2.width(), label2.height())
                    )
                if i == 1:
                    label2.setPixmap(
                        QtGui.QPixmap("gem2.png").scaled(
                            label2.width(), label2.height()
                        )
                    )
                if i == 2:
                    label2.setPixmap(
                        QtGui.QPixmap("gem3.png").scaled(
                            label2.width(), label2.height()
                        )
                    )
                if i == 3:
                    label2.setPixmap(
                        QtGui.QPixmap("gem4.png").scaled(
                            label2.width(), label2.height()
                        )
                    )
        ...
        for i in range(14):
            ...
            label2.setGeometry(
                QtCore.QRect(360 + leftUp[i][1] - 10, 8 + leftUp[i][0] - 5, 50, 50)
            )
            pe = QtGui.QPalette()
    ...
    def generatePosition(self, target):
        ...
        if (target == 6) or (target == 13):
            maxy = (
                base[1] + leftUp[target][0] - commanderSize[0] / 2 + int(height1 * 1.3)
            )
        else:
            maxy = (
                base[1] + leftUp[target][0] - commanderSize[0] / 2 + int(height2 * 1.3)
            )
